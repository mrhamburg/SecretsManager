...
public sealed class AliyunKmsSecretProviderTests
{
    private readonly IAliyunKmsApiClient _mockClient = Substitute.For<IAliyunKmsApiClient>();

    private AliyunKmsSecretProvider CreateProvider() => new(_mockClient);

    private static AliyunSecretValueResult MakeSecretValue(
        string name = "my-secret",
        string data = "secret-value",
        string versionId = "v1",
        DateTimeOffset? createTime = null) =>
        new(name, data, versionId, "text", createTime ?? DateTimeOffset.Parse("2026-01-01T00:00:00Z"), ["ACSCurrent"]);
    ...
    {
        _mockClient.GetSecretValueAsync("my-secret", null, "v2", Arg.Any<CancellationToken>())
        ...
        await _mockClient.Received(1).GetSecretValueAsync("my-secret", null, "v2", Arg.Any<CancellationToken>());
    }
    ...
    public async Task GetSecretAsync_NotFound_ThrowsSecretNotFoundException()
    {
        _mockClient.GetSecretValueAsync("missing", null, null, Arg.Any<CancellationToken>())
            .Returns((AliyunSecretValueResult?)null);
    ...
    {
        _mockClient.GetSecretValueAsync("new-secret", null, null, Arg.Any<CancellationToken>())
            .Returns((AliyunSecretValueResult?)null);

        _mockClient.CreateSecretAsync("new-secret", "new-value", Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(new AliyunSecretResult("new-secret", "v-abc123", "Generic"));
    ...
    {
        _mockClient.GetSecretValueAsync("existing", null, null, Arg.Any<CancellationToken>())
        ...
        _mockClient.PutSecretValueAsync("existing", "updated-value", Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(new AliyunSecretResult("existing", "v-new", "Generic"));
    ...
    public async Task GetSecretVersionsAsync_ReturnsAllVersions()
    {
        _mockClient.ListSecretVersionIdsAsync("versioned", Arg.Any<CancellationToken>())
            .Returns([
                new AliyunVersionResult("v1", DateTimeOffset.Parse("2026-01-01T00:00:00Z"), ["ACSPrevious"]),
                new AliyunVersionResult("v2", DateTimeOffset.Parse("2026-01-02T00:00:00Z"), ["ACSCurrent"])
            ]);
    ...
    {
        _mockClient.GetSecretValueAsync("exists", null, null, Arg.Any<CancellationToken>())
