...
public sealed class IBMCloudSecretsManagerSecretProviderTests
{
    private readonly IIBMCloudSecretsManagerApiClient _mockClient = Substitute.For<IIBMCloudSecretsManagerApiClient>();

    private IBMCloudSecretsManagerSecretProvider CreateProvider() => new(_mockClient);

    private static IBMCloudSecretsManagerSecretResult MakeSecret(string id = "secret-id-1", string name = "my-secret") =>
        new(id, name,
    ...
    public async Task GetSecretAsync_ReturnsSecretValue()
    {
        ...
        _mockClient.AccessVersionAsync("secret-id-1", "latest", Arg.Any<CancellationToken>())
            .Returns(new IBMCloudSecretsManagerAccessResult("secret-id-1", 2, "secret-value"));
    ...
    public async Task GetSecretAsync_WithVersion_PassesRevisionToClient()
    {
        ...
        _mockClient.AccessVersionAsync("secret-id-1", "1", Arg.Any<CancellationToken>())
            .Returns(new IBMCloudSecretsManagerAccessResult("secret-id-1", 1, "old-value"));
    ...
    public async Task GetSecretAsync_WithProperty_ExtractsJsonProperty()
    {
        ...
        _mockClient.AccessVersionAsync("secret-id-1", "latest", Arg.Any<CancellationToken>())
            .Returns(new IBMCloudSecretsManagerAccessResult("secret-id-1", 1, """{"host":"db.example.com","port":5432}"""));
    ...
    public async Task GetSecretAsync_WithNestedProperty_ExtractsNestedValue()
    {
        ...
        _mockClient.AccessVersionAsync("secret-id-1", "latest", Arg.Any<CancellationToken>())
            .Returns(new IBMCloudSecretsManagerAccessResult("secret-id-1", 1, """{"db":{"host":"db.example.com"}}"""));
    ...
    public async Task GetSecretAsync_NotFound_ThrowsSecretNotFoundException()
    {
        _mockClient.FindSecretByNameAsync("missing", Arg.Any<CancellationToken>())
            .Returns((IBMCloudSecretsManagerSecretResult?)null);
    ...
    public async Task PutSecretAsync_SecretDoesNotExist_CreatesSecretAndVersion()
    {
        ...
        _mockClient.CreateVersionAsync("secret-id-1", "new-value", Arg.Any<CancellationToken>())
            .Returns(new IBMCloudSecretsManagerVersionResult(1, "secret-id-1", "enabled",
    ...
    public async Task PutSecretAsync_SecretExists_CreatesNewVersionOnly()
    {
        ...
        _mockClient.CreateVersionAsync("secret-id-1", "updated-value", Arg.Any<CancellationToken>())
            .Returns(new IBMCloudSecretsManagerVersionResult(3, "secret-id-1", "enabled",
                DateTimeOffset.Parse("2026-03-01T00:00:00Z"), null, true));
    ...
    public async Task GetSecretVersionsAsync_ReturnsAllVersionsWithCurrentMarked()
    {
        ...
        _mockClient.ListVersionsAsync("secret-id-1", Arg.Any<CancellationToken>())
            .Returns([
                new IBMCloudSecretsManagerVersionResult(1, "secret-id-1", "enabled",
                    DateTimeOffset.Parse("2026-01-01T00:00:00Z"), null, false),
                new IBMCloudSecretsManagerVersionResult(2, "secret-id-1", "enabled",
                    DateTimeOffset.Parse("2026-02-01T00:00:00Z"), null, false),
                new IBMCloudSecretsManagerVersionResult(3, "secret-id-1", "enabled",
                    DateTimeOffset.Parse("2026-03-01T00:00:00Z"), null, true)
            ]);
